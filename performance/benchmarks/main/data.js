window.BENCHMARK_DATA = {
  "lastUpdate": 1754427497667,
  "repoUrl": "https://github.com/slatedb/slatedb",
  "entries": {
    "slatedb-bencher/benchmark-db.sh": [
      {
        "commit": {
          "author": {
            "name": "flaneur",
            "username": "flaneur2020",
            "email": "me.ssword@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "2fe991affaed8e2cb9caf3eed73589367484f09a",
          "message": "Add transaction RFC (#260)\n\nThis RFC specifies the design for adding transactional support to SlateDB by:\n\n- Defining a Transaction API aligned with Snapshot and WriteBatch (get, put, delete, scan, commit, rollback)\n- Specifying isolation levels (Snapshot Isolation and Serializable Snapshot Isolation) with optimistic conflict checking via a global Oracle\n- Outlining a GC and expiration strategy to bound Oracle state and avoid floating garbage\n- Comparing transaction approaches in RocksDB and Badger\n- Laying out an implementation roadmap: WriteBatch → Snapshot → SI → SSI",
          "timestamp": "2025-07-10T02:20:10Z",
          "url": "https://github.com/slatedb/slatedb/commit/2fe991affaed8e2cb9caf3eed73589367484f09a"
        },
        "date": 1752223252859,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5277.083,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5242.683,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4115.833,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1029.117,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1387.72,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 346.74,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2780.25,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1851.367,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1424.06,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 951.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2142.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3214.133,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1475.54,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2202.28,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1551.5,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6230.133,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1464.88,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5840.02,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "flaneur",
            "username": "flaneur2020",
            "email": "me.ssword@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "2fe991affaed8e2cb9caf3eed73589367484f09a",
          "message": "Add transaction RFC (#260)\n\nThis RFC specifies the design for adding transactional support to SlateDB by:\n\n- Defining a Transaction API aligned with Snapshot and WriteBatch (get, put, delete, scan, commit, rollback)\n- Specifying isolation levels (Snapshot Isolation and Serializable Snapshot Isolation) with optimistic conflict checking via a global Oracle\n- Outlining a GC and expiration strategy to bound Oracle state and avoid floating garbage\n- Comparing transaction approaches in RocksDB and Badger\n- Laying out an implementation roadmap: WriteBatch → Snapshot → SI → SSI",
          "timestamp": "2025-07-10T02:20:10Z",
          "url": "https://github.com/slatedb/slatedb/commit/2fe991affaed8e2cb9caf3eed73589367484f09a"
        },
        "date": 1752309503621,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5193.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5681.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4013.333,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1007.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1463.64,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 372,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2738.567,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1834.917,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1418.58,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 949.08,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2068.367,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3094.467,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1472.12,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2200.88,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1737.14,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6940.1,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1362.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5451.16,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "flaneur",
            "username": "flaneur2020",
            "email": "me.ssword@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "2fe991affaed8e2cb9caf3eed73589367484f09a",
          "message": "Add transaction RFC (#260)\n\nThis RFC specifies the design for adding transactional support to SlateDB by:\n\n- Defining a Transaction API aligned with Snapshot and WriteBatch (get, put, delete, scan, commit, rollback)\n- Specifying isolation levels (Snapshot Isolation and Serializable Snapshot Isolation) with optimistic conflict checking via a global Oracle\n- Outlining a GC and expiration strategy to bound Oracle state and avoid floating garbage\n- Comparing transaction approaches in RocksDB and Badger\n- Laying out an implementation roadmap: WriteBatch → Snapshot → SI → SSI",
          "timestamp": "2025-07-10T02:20:10Z",
          "url": "https://github.com/slatedb/slatedb/commit/2fe991affaed8e2cb9caf3eed73589367484f09a"
        },
        "date": 1752395892373,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5341.817,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 6098.883,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4910.233,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1233.483,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1473.64,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 369.1,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2589.917,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1717.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1421.5,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 951.36,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1917.483,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2888.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1398.72,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2105.12,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1498.967,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5968.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1137.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 4529.333,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "KarinaMilet",
            "username": "KarinaMilet",
            "email": "108008166+KarinaMilet@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "0235383af56b92fe866351c7b431b14b7884b699",
          "message": "docs(db_cache): add comments to `foyer_hybrid.rs` (#678)",
          "timestamp": "2025-07-14T03:09:38Z",
          "url": "https://github.com/slatedb/slatedb/commit/0235383af56b92fe866351c7b431b14b7884b699"
        },
        "date": 1752482720562,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1487.55,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 2151.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 3063.283,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 761.333,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1328.66,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 337.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1809.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1216.017,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1308.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 881.16,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1377.817,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2067.317,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1301.94,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1946.62,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1366.88,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5496.46,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1300.72,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5192.1,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "David Calavera",
            "username": "calavera",
            "email": "1050+calavera@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "56edc2d1fd5b5d1c37d78963e97897ee83250c6f",
          "message": "fix: Use max_concurrent_compactions in size tiered compaction. (#675)\n\nUse the option in the configuration to check if we can pick the next\ncompaction.\n\nFixes https://github.com/slatedb/slatedb/issues/653",
          "timestamp": "2025-07-15T03:25:04Z",
          "url": "https://github.com/slatedb/slatedb/commit/56edc2d1fd5b5d1c37d78963e97897ee83250c6f"
        },
        "date": 1752568962158,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1781.483,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 1746.433,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 3038.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 760.733,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1335.52,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 335.06,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1802.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1200.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1312.18,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 877,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1324.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 1990.633,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1301.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1936.88,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1214.567,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 4859.5,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1300.56,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5222.64,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "5357b451e11b756de35675bad778f5c54ae110e7",
          "message": "Protected against key/value length overflows in `SstRowCodecV0` (#679)\n\nSlateDB's block format only allows u16::MAX sized keys[^1] and u32::MAX\nsized values.\n\nThe `Db` currently allows users to put() with keys that are > u16 and\nvalues that are > u32. The `SstRowCodecV0` happily accepts these rows\nand casts the lengths (which are `usize` for developer ergonomics) with\n`as u16` and `as u32`. This will silently overflow lengths. The outcome\nis that future decodes will fail in odd ways (I was seeing `Error:\nInvalidRowFlags { encoded_bits: 244, known_bits: 15, message: \"Unable to\nparse flags. This may be caused by reading data encoded with a newer\ncodec.\" }`).\n\nThis PR adds protection both at the `WriteBatch` level and the\n`SstRowCodecV0` level. It also adds tests to validate things work as\nexpected. I opted to do the validation as a `panic` (via `assert`) in\nthe `WriteBatch` `put`/`delete` calls since a) it's in keeping with our\ncurrent `key.is_empty()` check, and b) we don't expose return a `Result`\nfor the methods (I didn't want to do a bw-incompatible change for this).\n\nI also briefly considered doing the check in `batch_write.rs`, but that\nwould a) burn CPU on the writer thread, b) defer the failure until\n`write` is called rather than exposing the issue immediately on\n`batch.put()`, and c) could leave us with a partially written batch\nunless we fully validate before writing (c.f. cpu burn). The advantage\nwould be that we could expose a error as a `SlateDbError` rather than\npanicking. I don't the tradeoffs are worth it.\n\n[^1]: _Technically_ we allow up to a max of u16 prefix bytes and u16\nsuffix bytes. But since the prefix can always be 0, we have to cap the\nsum of the two at 16.",
          "timestamp": "2025-07-15T17:45:47Z",
          "url": "https://github.com/slatedb/slatedb/commit/5357b451e11b756de35675bad778f5c54ae110e7"
        },
        "date": 1752655412394,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1903.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 2070.383,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 2456,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 613.483,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1325.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 336.84,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1791.667,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1191.283,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1311.26,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 881.76,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1434.767,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2166.65,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1302.72,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1953.22,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1424.12,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5693.66,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1300.16,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5217.96,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "David Calavera",
            "username": "calavera",
            "email": "1050+calavera@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "2542e7d3b9b96b0ac403d9ce24d916b7374186a4",
          "message": "Record bytes_compacted correctly. (#682)\n\nWhen a writer is finished, the compaction process is recording 0 as the\nsize, instead of the size of the current bytes processed until that\npoint.\n\nI was looking at these two lines while trying to implement #676, and it\nseems wrong. Since this is a small change, I rather get it out while I\ntry to figure out how to implement #676 correctly.",
          "timestamp": "2025-07-16T21:44:11Z",
          "url": "https://github.com/slatedb/slatedb/commit/2542e7d3b9b96b0ac403d9ce24d916b7374186a4"
        },
        "date": 1752741801752,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1225.633,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 2130.533,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 3065.267,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 773.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1321.34,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 330.98,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1833.017,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1215.983,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1309.12,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 871.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1422.383,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2138.067,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1303.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1975.56,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1379.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5509.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1103.483,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 4408.833,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "flaneur",
            "username": "flaneur2020",
            "email": "me.ssword@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "e318f5d191bcb212189dfceb13a52b1d2c9140c5",
          "message": "feat(compaction): add retention policy for snapshot versioned data (#638)\n\nlsm engines like leveldb/rocksdb mostly takes a reference counting\napproach to manage the active snapshots.\n\nin slatedb, we have a single writer / multiple reader architecture, as a\nresult, we can not track the active snapshots in a single process. to\nallow having Snapshots or TimeTravel, we can consider having a\ntime-retention based approach, like:\nhttps://www.tabular.io/apache-iceberg-cookbook/data-operations-snapshot-expiration/\n\nthis pr plans to add a PoC about this time-based retention support.\n\nit should make some modifications over the compaction phase. currently,\nthe compaction phase is:\n\n1. we use MergeIterator to iterate the latest versioned key/values.\n2. filter out the expired/tombstoned key/values (if it's in last level).\n3. append to the new ssts.\n\nwith retention in consideration, it will become:\n\n1. iterate the multiple versions of a single key in the decreasing order\nof seqnum.\n2. if a key has the multiple versions, filter out the versions whose\ncreate_time is earlier than the retention time.\n3. if a key has only one version, and expired/tombstoned, skip this\nkey/value (if it's last level)\n4. append to the new ssts.\n\nto manage this retention logic, this pr adds a RetentionBuffer for this,\nit works as a state machine to:\n\n1. collect the multiple versions of the same key\n2. when all the versions has been collected, handle the retention policy\n3. after retention policy as applied to the versions, pop one version at\na time\n\nthis algorithm is not optimal on memory, but it's easier to reason about\nthe correctness of implementation. (we can do optimizations after we\nreviewed that this handling logic has no issues.)\n\nafter this change, the expiry & tombstone handling in compactor.rs also\nhas to be multi-version aware, so this PR moves the expiry & tombstone\nhandling logic into retention_iterator.rs as well.",
          "timestamp": "2025-07-18T01:07:27Z",
          "url": "https://github.com/slatedb/slatedb/commit/e318f5d191bcb212189dfceb13a52b1d2c9140c5"
        },
        "date": 1752856537374,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1235.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 2143.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 2978.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 744.333,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1330.26,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 331.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1792.85,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1188.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1308.62,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 868.84,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1420.267,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2123.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1303.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1948.96,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1210.683,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 4856.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1300.52,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5200.02,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "9f58bf62f2b9e6883e450ea7a9b837daa39ce7f9",
          "message": "Remove tokio::time usage (#683)\n\nI thought it would be OK to continue to use `tokio::time` directly in\nSlateDB since we can pause/resume it in tests (e.g. with\n`start_paused=true`. Unfortunately, I found an issue with that when\nworking on some deterministic simulation tests. The issue arises if you\nwant to run a simulation test twice as though it were the same point in\ntime. Unfortunately, Tokio's `time` module does not allow users to set a\ntime in the past, only to advance it.\n\nThis PR goes through the code to replace (almost) all usage of\n`tokio::time` methods and types with our `SystemClock` trait. To\naccomplish all that tokio does for us, I had to introduce some new\nmethods to our system clock: advance, sleep, and ticker. I opted to\nreplace`timeout` usage witha `tokio::select!` rather than fight with\nRust's lifetime stuff more. If people strongly object lacking a\n`timeout` method, I can revisit it.\n\nThere are a few places where `tokio::time` is still used. All are in\ntests, and they're fairly harmless. Some day maybe we can scrub the\nremaining ones, but this PR is already quite large.\n\nThis PR also introduces `MockSystemClock`, which is a mock clock that's\ncompletely divorced from Tokio's time as well as std::time. It's\nentirely self-encapsulated, and allows users to set any time they want.\nThis is the clock I use in my DST branch, and it seems to work quite\nnicely.\n\nI've also added tests for both the default and mock system clocks.\n\nNote that the mock clock is only available when SlateDB's `test-util`\nfeature is enabled (this is a new feature in this PR). I had to go this\nroute because DST is done in the `slatedb-dst` crate, which only has\naccess to public APIs in SlateDB. Otherwise, I would have simply used\n`#[cfg(test)]`. The `test-util` follows Tokio's pattern as well.",
          "timestamp": "2025-07-18T21:18:55Z",
          "url": "https://github.com/slatedb/slatedb/commit/9f58bf62f2b9e6883e450ea7a9b837daa39ce7f9"
        },
        "date": 1752914441070,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1593.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 1713.867,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 2359.467,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 590.983,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1331.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 329.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1485.583,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 990.333,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1307.76,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 875.98,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1223.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 1832.383,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1303.24,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1962.24,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1156,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 4612.833,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1300.98,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5225.16,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "9f58bf62f2b9e6883e450ea7a9b837daa39ce7f9",
          "message": "Remove tokio::time usage (#683)\n\nI thought it would be OK to continue to use `tokio::time` directly in\nSlateDB since we can pause/resume it in tests (e.g. with\n`start_paused=true`. Unfortunately, I found an issue with that when\nworking on some deterministic simulation tests. The issue arises if you\nwant to run a simulation test twice as though it were the same point in\ntime. Unfortunately, Tokio's `time` module does not allow users to set a\ntime in the past, only to advance it.\n\nThis PR goes through the code to replace (almost) all usage of\n`tokio::time` methods and types with our `SystemClock` trait. To\naccomplish all that tokio does for us, I had to introduce some new\nmethods to our system clock: advance, sleep, and ticker. I opted to\nreplace`timeout` usage witha `tokio::select!` rather than fight with\nRust's lifetime stuff more. If people strongly object lacking a\n`timeout` method, I can revisit it.\n\nThere are a few places where `tokio::time` is still used. All are in\ntests, and they're fairly harmless. Some day maybe we can scrub the\nremaining ones, but this PR is already quite large.\n\nThis PR also introduces `MockSystemClock`, which is a mock clock that's\ncompletely divorced from Tokio's time as well as std::time. It's\nentirely self-encapsulated, and allows users to set any time they want.\nThis is the clock I use in my DST branch, and it seems to work quite\nnicely.\n\nI've also added tests for both the default and mock system clocks.\n\nNote that the mock clock is only available when SlateDB's `test-util`\nfeature is enabled (this is a new feature in this PR). I had to go this\nroute because DST is done in the `slatedb-dst` crate, which only has\naccess to public APIs in SlateDB. Otherwise, I would have simply used\n`#[cfg(test)]`. The `test-util` follows Tokio's pattern as well.",
          "timestamp": "2025-07-18T21:18:55Z",
          "url": "https://github.com/slatedb/slatedb/commit/9f58bf62f2b9e6883e450ea7a9b837daa39ce7f9"
        },
        "date": 1753000790244,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 744.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 1877.917,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 2304.733,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 570.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1321.54,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 327.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1427.35,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 943.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1307.44,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 876.32,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1252.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 1877.683,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1302.14,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1946.26,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1131.133,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 4526.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1084.183,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 4316.4,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "9f58bf62f2b9e6883e450ea7a9b837daa39ce7f9",
          "message": "Remove tokio::time usage (#683)\n\nI thought it would be OK to continue to use `tokio::time` directly in\nSlateDB since we can pause/resume it in tests (e.g. with\n`start_paused=true`. Unfortunately, I found an issue with that when\nworking on some deterministic simulation tests. The issue arises if you\nwant to run a simulation test twice as though it were the same point in\ntime. Unfortunately, Tokio's `time` module does not allow users to set a\ntime in the past, only to advance it.\n\nThis PR goes through the code to replace (almost) all usage of\n`tokio::time` methods and types with our `SystemClock` trait. To\naccomplish all that tokio does for us, I had to introduce some new\nmethods to our system clock: advance, sleep, and ticker. I opted to\nreplace`timeout` usage witha `tokio::select!` rather than fight with\nRust's lifetime stuff more. If people strongly object lacking a\n`timeout` method, I can revisit it.\n\nThere are a few places where `tokio::time` is still used. All are in\ntests, and they're fairly harmless. Some day maybe we can scrub the\nremaining ones, but this PR is already quite large.\n\nThis PR also introduces `MockSystemClock`, which is a mock clock that's\ncompletely divorced from Tokio's time as well as std::time. It's\nentirely self-encapsulated, and allows users to set any time they want.\nThis is the clock I use in my DST branch, and it seems to work quite\nnicely.\n\nI've also added tests for both the default and mock system clocks.\n\nNote that the mock clock is only available when SlateDB's `test-util`\nfeature is enabled (this is a new feature in this PR). I had to go this\nroute because DST is done in the `slatedb-dst` crate, which only has\naccess to public APIs in SlateDB. Otherwise, I would have simply used\n`#[cfg(test)]`. The `test-util` follows Tokio's pattern as well.",
          "timestamp": "2025-07-18T21:18:55Z",
          "url": "https://github.com/slatedb/slatedb/commit/9f58bf62f2b9e6883e450ea7a9b837daa39ce7f9"
        },
        "date": 1753087559212,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5409.733,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5546.633,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4222.567,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1057.533,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1170.85,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 294.317,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2671.25,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1784.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1331.86,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 886.72,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1091.25,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 1628.267,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1394.86,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2077.46,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1742.58,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6959.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1466.02,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5877.02,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "930a06c49ce95224b89e776ba91ce34e26d941e3",
          "message": "Empty scan ranges cause panic (#681)\n\nSST iterators currently make this check when there are no more fetch\ntasks:\n\n```rust\nassert_eq!(self.next_block_idx_to_fetch, self.block_idx_range.end);\n```\n\nThis makes sense in the happy path case where we've traversed blocks in\norder. There's a pathological case that causes this code to panic. If a\nuser scans in such a way that the range's start key > the range's end\nkey, it causes a panic here with something like:\n\n```\n---- sst_iter::tests::test_iter_with_start_key_larger_than_end_key stdout ----\n\nthread 'sst_iter::tests::test_iter_with_start_key_larger_than_end_key' panicked at slatedb/src/sst_iter.rs:295:17:\nassertion `left == right` failed\n  left: 2\n right: 1\nnote: run with `RUST_BACKTRACE=1` environment variable to display a backtrace\n\n\nfailures:\n    sst_iter::tests::test_iter_with_start_key_larger_than_end_key\n```\n\nI was getting this while doing deterministic simulation testing. I've\nadded a unit test to reproduce and updated the check to allow start\nblocks to be `>=` end blocks.",
          "timestamp": "2025-07-21T20:50:04Z",
          "url": "https://github.com/slatedb/slatedb/commit/930a06c49ce95224b89e776ba91ce34e26d941e3"
        },
        "date": 1753346596897,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 4349.817,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 4564.833,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 3687.1,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 916.95,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1368.5,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 337.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2722.617,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1812.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1323.74,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 884.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2014.867,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3036.217,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1393.12,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2080.62,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1422.633,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5676.683,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1377.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5487.22,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "flaneur",
            "username": "flaneur2020",
            "email": "me.ssword@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "1d8303797e3bdbf515727e104f95dff8345521e5",
          "message": "chore: rename the ReadSnapshot trait to DbStateReader (#693)\n\nduring the implementation of #688, I came across the following code\nwhere the term snapshot appears to be overloaded after `DbSnapshot`\nintroduced:\n\n```\n    pub async fn get_with_options<K: AsRef<[u8]>>(\n        &self,\n        key: K,\n        options: &ReadOptions,\n    ) -> Result<Option<Bytes>, SlateDBError> {\n        self.db_stats.get_requests.inc();\n        self.check_error()?;\n        let snapshot = self.state.read().snapshot();\n        self.reader\n            .get_with_options(key, options, &snapshot, None)\n            .await\n    }\n```\n\nthis PR tries to rename `self.state.read().snapshot()` to\n`self.state.read().view()` to represent \"a consistent view of the\ndatabase state\" and avoid naming conflicts with the \"DbSnapshot\"\nconcept.\n\ni am still a bit debating if `DbStateView` and `DbStateReader` here are\nbetter naming though.. any feedback is appreciated!",
          "timestamp": "2025-07-24T22:25:10Z",
          "url": "https://github.com/slatedb/slatedb/commit/1d8303797e3bdbf515727e104f95dff8345521e5"
        },
        "date": 1753433024436,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1094.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 1421.767,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 3234.533,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 803.117,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1335.78,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 334.62,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1591.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1064.767,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1307,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 876.94,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1450.35,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2172.25,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1302.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1943.98,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1188,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 4739.983,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1300.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5199.9,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "flaneur",
            "username": "flaneur2020",
            "email": "me.ssword@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "1d8303797e3bdbf515727e104f95dff8345521e5",
          "message": "chore: rename the ReadSnapshot trait to DbStateReader (#693)\n\nduring the implementation of #688, I came across the following code\nwhere the term snapshot appears to be overloaded after `DbSnapshot`\nintroduced:\n\n```\n    pub async fn get_with_options<K: AsRef<[u8]>>(\n        &self,\n        key: K,\n        options: &ReadOptions,\n    ) -> Result<Option<Bytes>, SlateDBError> {\n        self.db_stats.get_requests.inc();\n        self.check_error()?;\n        let snapshot = self.state.read().snapshot();\n        self.reader\n            .get_with_options(key, options, &snapshot, None)\n            .await\n    }\n```\n\nthis PR tries to rename `self.state.read().snapshot()` to\n`self.state.read().view()` to represent \"a consistent view of the\ndatabase state\" and avoid naming conflicts with the \"DbSnapshot\"\nconcept.\n\ni am still a bit debating if `DbStateView` and `DbStateReader` here are\nbetter naming though.. any feedback is appreciated!",
          "timestamp": "2025-07-24T22:25:10Z",
          "url": "https://github.com/slatedb/slatedb/commit/1d8303797e3bdbf515727e104f95dff8345521e5"
        },
        "date": 1753519195452,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5458.683,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5256.85,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 3411.667,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 853.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1471.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 369.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2876.433,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1914.983,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1408.62,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 941.52,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2065.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3109.35,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1406.88,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2109.86,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1663.52,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6661.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1375.18,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5489.98,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "David Calavera",
            "username": "calavera",
            "email": "1050+calavera@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "185cf15fd7e21c2776a975456f51341cf2e418ae",
          "message": "fix: Use finished blocks length to calculate the sst current size. (#676)\n\nThis change takes into account that blocks can be compressed when they\nare finished to calculate the current size of a compacted SST.\n\nFixes https://github.com/slatedb/slatedb/issues/659",
          "timestamp": "2025-07-27T02:46:43Z",
          "url": "https://github.com/slatedb/slatedb/commit/185cf15fd7e21c2776a975456f51341cf2e418ae"
        },
        "date": 1753605571918,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 4753.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5713.5,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4203.817,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1052.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1476.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 370,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2586.65,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1735.633,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1413.56,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 943.72,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1894.267,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2839.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1395.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2091.68,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1467.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5874.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1466.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5862.3,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "394454b63a0d7d291611f304821206bdc66f8812",
          "message": "Add `gen_ulid` clock param (#700)\n\nI found that our current ULID generator is non-deterministic because it\nuses the `ulid` crate to get time, which always uses system time, not\nour `SystemClock`. Fixed this by adding a param to take our clock.\n\nI considered separating ULID and UUID generation out so we can have a\nULID struct that contains a clock, but it felt clunky to have two\ndifferent ways to generate IDs. This way, the ULID and UUID generation\ncontinue to piggyback off of any `RngCore`, and we only need to pass a\nclock when we're generating ULIDs, which is relatively limited.",
          "timestamp": "2025-07-28T08:27:07Z",
          "url": "https://github.com/slatedb/slatedb/commit/394454b63a0d7d291611f304821206bdc66f8812"
        },
        "date": 1753692325059,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 3486.083,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 4737.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4129.417,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1037.783,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1486.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 368.74,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2799.483,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1874.583,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1426.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 945.92,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1904.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2852.017,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1391.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2086.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1571.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6258.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1468.02,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5836.9,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "8ad02ce9d566343d4cb28a65da0da7f30e9a363d",
          "message": "Add `flush_with_options` to enable manual memtable flushing (#699)",
          "timestamp": "2025-07-28T14:18:09Z",
          "url": "https://github.com/slatedb/slatedb/commit/8ad02ce9d566343d4cb28a65da0da7f30e9a363d"
        },
        "date": 1753778594013,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5142.583,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5090.783,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4946.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1236.367,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1462.16,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 370.64,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2781.567,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1854.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1406.94,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 931.02,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1965.883,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2953.667,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1315.68,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1977.08,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1591.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6314.183,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1160.683,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 4645.283,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "abaa94a82789bfd2865d331740e9ac4233e1d534",
          "message": "Replace `tracing::` logs with `log::` logs in `slatedb` (#702)\n\nWe should be using the `logs` facade crate everywhere in `slatedb` since\nit's a library. I added clippy macro checks to enforce this and updated\nthe code accordingly.\n\nI left the tracing_subscriber (vs. env_logger or others) for output in\ntests and tools since that's what `opendal` does and I didn't want to\noverthink things. I also left the -cli and -bencher crates to continue\nusing `tracing` for their logs since they're applications, not\nlibraries.\n\nI also took the opportunity to prevent...\n\n- stdout print's\n- Option::unwrap\n- Result::unwrap",
          "timestamp": "2025-07-29T18:51:41Z",
          "url": "https://github.com/slatedb/slatedb/commit/abaa94a82789bfd2865d331740e9ac4233e1d534"
        },
        "date": 1753865021414,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 4513.583,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5817.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4720.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1185.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1262.467,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 321.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2802.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1857.933,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1333.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 883.34,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2037.933,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3066.667,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1406.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2105.98,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1776.68,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 7121.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1383.66,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5554.26,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "82847c76d07c4701cc63994091bcf087bb113c68",
          "message": "Add deterministic simulation tests to SlateDB (#694)\n\nA `slatedb-dst` package is added in this PR. The package contains\ndst.rs, which contains a deterministic simulator that runs random\noperations against a SlateDB instance and verifies that the DB has the\ncorrect data. Several integration tests are available in\n`simulation.rs`; they've been added to the `pr.yaml` check, too. See the\nREADME.md, dst.rs docs, and simulations.rs docs for usage and\nimplementation details.\n\nThere is still much to be done to improve the simulation tests:\n\n- Add unit tests to dst.rs to verify method behavior.\n- Add a CLI to trigger DST's outside of `cargo`\n- Incorporate DSTs into the `nightly.yaml`\n- Complete the `TODO`s to add more randomness and configuration\n- Add TTL, checkpointing, DbReader, and fencing (multi-writer), and DB\nclose/open checks and actions\n- Incorporate the object store file cache and inject faults in the\nfilesystem\n- Inject faults in the object store requests\n- Evaluate using proptest to build DB settings and generate random data\n- Verify log output is identical in `test_dst_is_deterministic` (we only\nverify final seed and times are identical)\n\nStill, I feel this is a good start and the PR is already fairly lengthy.\nI'd like to do the CLI and nightly.yaml as follow-ons. The remaining\nitems can be completed over time.\n\nWhile running the tests, I discovered two instances of non-determinism\nin SlateDB:\n\n- The compactor was using `spawn_blocking` to start and stop compactions\nwith the executor. I assumed `spawn_blocking` would be deterministic,\nbut it turns out not to be. It spawns outside the `current_thread` Tokio\nruntime and creates a non-deterministic race between `.await` calls in\nthe `current_thread` and those in the spawned thread. I fixed this by\nadding a branch for `#[cfg(dst)]` that uses tokio's `spawn`, which runs\non the runtime threads. I opted not to use `spawn` for production code\nbecause we can't trust pluggable executors not to block for a long time\non start/stop compaction calls; they could block our runtime threads.\n- ULID was using the RNG to deterministically generate the random\nportion of its bytes, but was still using its own clock for the\ntimestamp portion. I fixed this by adding a `timestamp_ms` field to\n`gen_ulid`.\n\nI found the following resources helpful when doing this work:\n\n- https://pierrezemb.fr/posts/tokio-hidden-gems/\n- https://www.reddit.com/r/rust/comments/1jr8ogo/comment/mle11wh/\n- https://s2.dev/blog/dst\n\nFixes #267",
          "timestamp": "2025-07-30T22:23:33Z",
          "url": "https://github.com/slatedb/slatedb/commit/82847c76d07c4701cc63994091bcf087bb113c68"
        },
        "date": 1753951442945,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 1963.383,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 3076.55,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 3114.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 788.183,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1100.133,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 277.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1811.167,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1205.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1307.96,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 870.32,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1381.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2071.85,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1088.817,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1662.183,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1255.95,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5016.683,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1301.46,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5232.5,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "3d8abfd1c2688d06196708695c8f98fa48a81aad",
          "message": "Prevent ticker starvation (#708)",
          "timestamp": "2025-07-31T21:00:42Z",
          "url": "https://github.com/slatedb/slatedb/commit/3d8abfd1c2688d06196708695c8f98fa48a81aad"
        },
        "date": 1754037790568,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 6430,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5546.633,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4709.817,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1169.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1469.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 368.98,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2804.767,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1867.967,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1422.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 950.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2098.267,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3157.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1387.78,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2089.5,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1589.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6365.183,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1468.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5903.06,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Almog Gavra",
            "username": "agavra",
            "email": "almog.gavra@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "86c59e55e1642a379db6021290eb607d068ee98b",
          "message": "[refactor] make modifying COW state easier (#709)",
          "timestamp": "2025-08-02T02:00:42Z",
          "url": "https://github.com/slatedb/slatedb/commit/86c59e55e1642a379db6021290eb607d068ee98b"
        },
        "date": 1754123959759,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5275.433,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 6223.05,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 5102.383,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1277.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1474.22,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 367.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2865.067,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1896.167,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1419.32,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 950.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1927.75,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2885.067,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1311.46,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1973.74,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1775.92,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 7100.56,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1378.44,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5484.76,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Almog Gavra",
            "username": "agavra",
            "email": "almog.gavra@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "86c59e55e1642a379db6021290eb607d068ee98b",
          "message": "[refactor] make modifying COW state easier (#709)",
          "timestamp": "2025-08-02T02:00:42Z",
          "url": "https://github.com/slatedb/slatedb/commit/86c59e55e1642a379db6021290eb607d068ee98b"
        },
        "date": 1754210373993,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5630.583,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5681.92,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4157.967,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1039.733,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1380.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 342.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2783.567,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1855.017,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1421.92,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 946.08,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1856.533,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2788.733,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1159.867,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1747.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1499.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5969.65,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1467.56,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5847.26,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Almog Gavra",
            "username": "agavra",
            "email": "almog.gavra@gmail.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "86c59e55e1642a379db6021290eb607d068ee98b",
          "message": "[refactor] make modifying COW state easier (#709)",
          "timestamp": "2025-08-02T02:00:42Z",
          "url": "https://github.com/slatedb/slatedb/commit/86c59e55e1642a379db6021290eb607d068ee98b"
        },
        "date": 1754297101381,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 6492.55,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5681.917,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4939.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1234.85,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1254.117,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 312.917,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2729,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1826.417,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1335.66,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 886.34,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2042.25,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3078.233,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1314.54,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1976.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1592.45,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6342.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1365.14,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5485.18,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "3ff84113771fff7f7b0d692d5686ab02a72a84a6",
          "message": "Add nightly ZeroFS integration test (#732)\n\nThis is a copy of #715. I moved the test from that PR into the\n`nightly.yaml` so we can validate ZeroFS works on a nightly basis. I\ndidn't feel it was appropriate to run the test on every PR.",
          "timestamp": "2025-08-04T22:10:13Z",
          "url": "https://github.com/slatedb/slatedb/commit/3ff84113771fff7f7b0d692d5686ab02a72a84a6"
        },
        "date": 1754346219544,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 6349.233,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5946.467,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4155.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1039.417,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1381.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 345.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2671.85,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1783.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1325.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 877.32,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2009.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3007.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1384.18,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2083.92,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1760.32,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 7061.26,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1466.04,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5819.22,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "acfdbe398d52e446f0dc42368b3a2ebbd520a997",
          "message": "Add a nightly DST test (#733)\n\nThis PR integrates the DST into the nightly Github workflow.\n\n- Adds a `test_dst_nightly` test to simulation.rs.\n- Adds a `slow` cfg to `Cargo.toml` to signal slow tests should be run.\n- Updates `nightly.yaml` to run the DST on a WarpBuild instance.\n- Adds a `DstDuration` param that can be either `Iterations` or\n`Duration` (wall-clock time) to the `run_simulation`. This allows us to\nrun simulations for a set amount of time rather than a set number of\niterations.\n- Update `nextest.toml` to include a new `dst-nightly` profile.\n- Update docs and README.md\n\n`test_dst_nightly` runs a separate DST on each core of the host machine.\nThe tests run in parallel for 50 minutes.\n\nFuture improvements:\n\n- Fail as soon as a handle finishes rather than joining them\nsequentially.\n- Make `test_dst_nightly` use an environment variable so the 50m\nduration isn't hard-coded.",
          "timestamp": "2025-08-05T01:16:52Z",
          "url": "https://github.com/slatedb/slatedb/commit/acfdbe398d52e446f0dc42368b3a2ebbd520a997"
        },
        "date": 1754357510150,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 533.1,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 2101.9,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 2209.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 556.433,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 990.05,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 247.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 1980.65,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1325.1,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1302.78,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 869.08,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1065.167,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1593.267,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1378.08,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5537.86,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1299.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5190.98,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "427d82f7b1645086529201541c0d729c610ccb46",
          "message": "Try to prevent test timeout due to silent process (#734)",
          "timestamp": "2025-08-05T03:01:29Z",
          "url": "https://github.com/slatedb/slatedb/commit/427d82f7b1645086529201541c0d729c610ccb46"
        },
        "date": 1754363730199,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5965.95,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5276.067,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4645.233,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1159.617,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1473.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 366.78,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2730.283,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1817.767,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1426.06,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 955.78,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1948.267,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2936.367,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1391.18,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2079.84,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1734,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6930.24,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1467.2,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5886.3,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "427d82f7b1645086529201541c0d729c610ccb46",
          "message": "Try to prevent test timeout due to silent process (#734)",
          "timestamp": "2025-08-05T03:01:29Z",
          "url": "https://github.com/slatedb/slatedb/commit/427d82f7b1645086529201541c0d729c610ccb46"
        },
        "date": 1754383421797,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 4848.25,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 6259.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4525.417,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1134.233,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1476.88,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 370.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2728.967,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1819.867,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1183.033,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 791.55,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2115.95,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3182.633,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1475.58,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2210.16,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1481.467,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5927.933,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1466.74,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5860.02,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "285fe90ae0b488c1bf82a4d402c531c4190cb0a9",
          "message": "Cap DST nightly at 90% CPU usage (#735)\n\nThe GH action runner is killing our DST tests for some reason. I'm not\ntotally clear why, but I'm attempting to give it some headroom.",
          "timestamp": "2025-08-05T16:37:16Z",
          "url": "https://github.com/slatedb/slatedb/commit/285fe90ae0b488c1bf82a4d402c531c4190cb0a9"
        },
        "date": 1754412642880,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 4194.8,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 5680.367,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4985.417,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1238.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1563.06,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 389.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2773.283,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1857.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1503.1,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 1000.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 1868.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 2796.833,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1318.64,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 1990.66,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1456.35,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 5884.233,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1466.46,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5854.14,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "ca88ec559ed347ccd7ab325476d9d6969d3fa9e0",
          "message": "Run nightly DST on ubuntu-latest (#736)\n\nNightly DSTs are still failing on WarpBuild with the core usage dropped\nto 90%. It ran a lot longer (16m instead of 5m), but still got killed.\nI'm switching to GH action runners to see if it's an issue with\nWarpBuild (or Hetzner) machines.",
          "timestamp": "2025-08-05T17:49:55Z",
          "url": "https://github.com/slatedb/slatedb/commit/ca88ec559ed347ccd7ab325476d9d6969d3fa9e0"
        },
        "date": 1754417077809,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 5277.583,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 6206.183,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4742.317,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1190.5,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1558.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 385.48,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2714.517,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1815.983,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1434.38,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 958.58,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2042.383,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3083.6,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1415.32,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2123.94,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1549.317,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6172.55,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1470.42,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5907.9,
            "unit": "ops/sec"
          }
        ]
      },
      {
        "commit": {
          "author": {
            "name": "Chris",
            "username": "criccomini",
            "email": "criccomini@users.noreply.github.com"
          },
          "committer": {
            "name": "GitHub",
            "username": "web-flow",
            "email": "noreply@github.com"
          },
          "id": "1fe578c9dafea325c62e1064677878701e2f98e2",
          "message": "Back to warpbuild for DST nightly test (#739)",
          "timestamp": "2025-08-05T20:44:37Z",
          "url": "https://github.com/slatedb/slatedb/commit/1fe578c9dafea325c62e1064677878701e2f98e2"
        },
        "date": 1754427497107,
        "tool": "customBiggerIsBetter",
        "benches": [
          {
            "name": "SlateDB 100% Puts 32 Threads - Puts/s",
            "value": 6901.4,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 32 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Puts/s",
            "value": 7082.817,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 100% Puts 1 Threads - Gets/s",
            "value": 0,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Puts/s",
            "value": 4694.15,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 32 Threads - Gets/s",
            "value": 1180.417,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Puts/s",
            "value": 1414.12,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 80% Puts 1 Threads - Gets/s",
            "value": 356.92,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Puts/s",
            "value": 2727.667,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 32 Threads - Gets/s",
            "value": 1829.983,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Puts/s",
            "value": 1257.767,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 60% Puts 1 Threads - Gets/s",
            "value": 841.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Puts/s",
            "value": 2023.783,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 32 Threads - Gets/s",
            "value": 3032.583,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Puts/s",
            "value": 1415.7,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 40% Puts 1 Threads - Gets/s",
            "value": 2119.3,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Puts/s",
            "value": 1578.983,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 32 Threads - Gets/s",
            "value": 6354.917,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Puts/s",
            "value": 1468.82,
            "unit": "ops/sec"
          },
          {
            "name": "SlateDB 20% Puts 1 Threads - Gets/s",
            "value": 5900.14,
            "unit": "ops/sec"
          }
        ]
      }
    ]
  }
}